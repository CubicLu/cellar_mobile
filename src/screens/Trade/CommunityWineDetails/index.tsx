...
import {COMMUNITY_WINE} from '../../../apollo/queries/wine';
...
import {CREATE_OFFER_TO_BUY} from '../../../apollo/mutations/trading';
import {
  CommunityDetailWineBody,
  WineDetailsPreviableImage,
  ConfirmationDialog,
  CommunityOfferListItem,
  HeaderWithChevron,
} from '../../../components';
...
const WineDetails: FC<Props> = ({navigation}) => {
  ...
  const [onSubmitOffersToSell] = useMutation(CREATE_OFFER_TO_BUY, {
  ...
  const {data: wineData, loading: loadWine, error: wineError} = useQuery(COMMUNITY_WINE, {
        ...
        {wineData && (
          <View style={flex1}>
            <FlatList<WineInTrade>
              ...
              data={wineData.winesInTrade.wineInTrade}
              ...
              renderItem={({item, index}) => (
                <>
                  ...
                  <CommunityOfferListItem
                    data={item}
                    marketPrice={wineData.wineV2.wine.generalPriceInfo.market.avg}
                    index={index}
              ...
              ListFooterComponent={
                <>
                  {wineData.winesInTrade.wineInTrade.length > 0 && (
...
export const CommunityWineDetailsScreen = WineDetails;
