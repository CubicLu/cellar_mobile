...
import {GET_LOCAL_FILTERS} from '../../../apollo/client/queries';
import {ADD_LOCAL_FILTER, CLEAR_ONE_LOCAL_FILTER} from '../../../apollo/client/mutations';
...
const FilterList: React.FC<InventoryProps> = ({navigation}) => {
  ...
  const [clearSelection] = useMutation(CLEAR_ONE_LOCAL_FILTER, {
    variables: {
      title,
    },
  });
  ...
  useEffect(() => {
    if (selectedFilters && selectedFilters.inventoryFilters.length > 0) {
      let inventoryFilters = selectedFilters.inventoryFilters;
        ...
        <FlatList
          ...
          renderItem={({item, index}) => {
            return (
              <FilterItemNewCell
                index={index}
                isActive={!!_.find(selectedList, item)}
                onPress={() => {
                  addLocalFilter({variables: {filter: item, category: title}});
                }}
                borderStyle={data.length - 1 === index ? borderWidth3 : borderWidth0}
                title={item.title}
              />
            );
          }}
...
export const InventoryFilterItems = FilterList;
...
const {flatContainer, container, searchContainer, listContainer, flex1, borderWidth0, borderWidth3} = stylesMain;
