...
import {ADD_LOCAL_WISHLIST_FILTER, CLEAR_ONE_LOCAL_WISHLIST_FILTER} from '../../../apollo/client/mutations';
...
const FilterList: React.FC<InventoryProps> = ({navigation}) => {
  ...
  const [clearSelection] = useMutation(CLEAR_ONE_LOCAL_WISHLIST_FILTER);

  let [titleFallback, setTitleFallback] = useState(null);

  useEffect(() => {
    if (selectedFilters && selectedFilters.wishlistFilters.length > 0) {
      let wishlistFilters = selectedFilters.wishlistFilters;
      setSelectedList(wishlistFilters.filter(el => el[title] !== selectedList[title])[0][title]);
    }
  }, [selectedFilters]);
...
export const WishlistFilterItems = FilterList;
