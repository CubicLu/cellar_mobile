...
import {GET_COMMUNITY_HISTORY} from '../../apollo/queries/wineHistory';
...
const CommunityDrinkHistory: FC<Props> = ({navigation}) => {
  ...
  const {data: drinkHistory, fetchMore, loading} = useQuery(GET_COMMUNITY_HISTORY, {
    onCompleted: data => {
      if (!loading && data.communityInventoryHistoryWines.data.length < FIRST) {
        setInvalidate(false);
  ...
  const [getWineDetails] = useLazyQuery(WINE, {
    onCompleted: data => {
      RNProgressHud.dismiss();
      console.log(Routes.drinkHistoryDetails.name);
    ...
    try {
      if (!loading && invalidate) {
        ...
        await fetchMore({
          variables: {
            //@ts-ignore
            skip: drinkHistory.communityInventoryHistoryWines.data.length,
            first: FIRST,
            q: debouncedSearch,
          },
          ...
          updateQuery: (previousResult, {fetchMoreResult}) => {
            //@ts-ignore
            if (fetchMoreResult.communityInventoryHistoryWines.data.length < FIRST) {
              setInvalidate(false);
              toggleLoadingFooter(false);
            }
            ...
            return {
              //@ts-ignore
              ...previousResult,
              communityInventoryHistoryWines: {
                data: [
                  ...previousResult.communityInventoryHistoryWines.data,
                  ...fetchMoreResult.communityInventoryHistoryWines.data,
                ],
              },
            };
          },
  ...
  const onPressSearch = useCallback(() => {
    ...
    if (!isSearchVisible) {
      ...
      (searchRef as any).current && (searchRef as any).current.focus();
    }
      ...
      <SafeAreaView style={flex1}>
        <ImageBackground source={photo.inventoryBg} style={imageBg}>
          ...
          <FlatList<any>
            ...
            data={drinkHistory ? drinkHistory.communityInventoryHistoryWines.data : []}
            ...
            renderItem={({item}) => {
              return (
                <HistoryListItem
                  ...
                  bubbleCount={item.countOfReviews}
...
export const CommunityDrinkHistoryScreen = CommunityDrinkHistory;
